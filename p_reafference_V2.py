...
ev_id = {'OVTK_GDF_Left': 6, 'OVTK_GDF_Right': 7}  # this is specificaly for tilman2
eventx = events[0][(events[0][:, 2] > 5) & (events[0][:, 2] < 8)] # this is specificaly for tilman2
epochs = mne.Epochs(raw_1, events=eventx, event_id=ev_id, tmin=-1, tmax=5, baseline=(-1, 0))
labels_1 = epochs.events[:, -1] - 6  # this is specificaly for tilman2
epochs_train = epochs.copy()
epochs_data = epochs.get_data()  # n_epochs x n_channels x n_times
epochs_data = epochs_data[:, :, int(sfreq):]  # omit the baseline
epochs_data = np.delete(epochs_data, ind_del, axis=1)
epochs_data_1 = filtfilt(b, a, epochs_data, axis=2)

# TODO: baseline correction after filtering?
...
ev_id = {'OVTK_GDF_Left': 5, 'OVTK_GDF_Right': 6}  # this is specificaly for tilman2
eventx = events[0][(events[0][:, 2] > 4) & (events[0][:, 2] < 7)] # this is specificaly for tilman2
epochs = mne.Epochs(raw_2, events=eventx, event_id=ev_id, tmin=-1, tmax=5, baseline=(-1, 0))
"""
is there a baseline correction?
tune the epochs
***********
"""
labels_2 = epochs.events[:, -1] - 5  # this is specificaly for tilman2
epochs_train = epochs.copy()
epochs_data = epochs.get_data()  # n_epochs x n_channels x n_times
epochs_data = epochs_data[:, :, int(sfreq):]  # omit the baseline
epochs_data = np.delete(epochs_data, ind_del, axis=1)
epochs_data_2 = filtfilt(b, a, epochs_data, axis=2)
...
random_state_nested = np.random.randint(2**32)
...
for i_comp, n_components in enumerate(range_n_components):
    ...
    for tr_idx, te_idx in cv_split:
        ...
        epochs_train_nested_new = csp.fit_transform(epochs_train, labels_train)
        epochs_test_nested_new = csp.fit_transform(epochs_test, labels_test)
        lda = LinearDiscriminantAnalysis()
...
for train_idx, test_idx in cv_split:
    ...
    for i_comp, n_components in enumerate(range_n_components):
        ...
        for tr_idx_nested, te_idx_nested in cv_split_nested:
            ...
            epochs_train_nested_new = csp.fit_transform(epochs_train_nested, labels_train_nested)
            epochs_test_nested_new = csp.fit_transform(epochs_test_nested, labels_test_nested)
            lda = LinearDiscriminantAnalysis()
